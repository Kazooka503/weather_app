      ...
      <main className="px-3 max-w-7xl mx-auto flex flex-col gap-9 w-full pb-10 pt-4">
        ...
        <section className="space-y-4">
          <div className="space-y-2">
            <span id="date" className="flex gap-1 text-2xl items-end">
              {format(parseISO(firstData?.dt_txt ??''), 'EEEE')} <span className="text-lg">{format(parseISO(firstData?.dt_txt ??''), 'dd.MM.yyyy')}</span>
            </span>
              ...
              <div className="flex gap-10 sm:gap-16 overflow-x-auto w-full justify-between pr-3">
                {data?.list.map((d, i)=>
                  ...
                  >
                    ...
                    <p className="">{convertKelvinToCelsius(d?.main.temp ?? 0)}°</p>
                  </div>
                )}
              </div>
            </Container>
          ...
          <div className="flex gap-4">
                ...
                <Container className="bg-yellow-300/80 px-6 justify-between overflow-x-auto">
                  <WeatherDetails visability={metersToKilometers(firstData?.visibility ?? 10000)} airPressure={`${firstData?.main.pressure} hPa`} 
                    humdity={`${firstData?.main.humidity}%`} sunrise={format(fromUnixTime(data?.city.sunrise ?? 1702949452), "H:mm")}
                    sunset={format(fromUnixTime(data?.city.sunset ?? 1702949452), "H:mm")} windSpeed={convertWindSpeed(firstData?.wind.speed ?? 1.64)}
                  />
                </Container>
              </div>
        </section>
        ...
        <section className="flex w-full flex-col gap-4">
          ...
          {firstDataForEachDate.map((d, i) =>
            ...
            humdity={`${d?.main.humidity}%`}
            sunrise={format(
              fromUnixTime(data?.city.sunrise ?? 1702517657), "H:mm"
            )}
            sunset={format(
              fromUnixTime(data?.city.sunset ?? 1702517657), "H:mm"
            )}
            visability={`${metersToKilometers(d?.visibility ?? 100000)}`}
